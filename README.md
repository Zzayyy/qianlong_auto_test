...
面向 Windows 期权交易客户端的 GUI 自动化项目，覆盖**查询、下单、撤单、组合申报、超级策略一键开仓、交易系统设置自动检查**，并通过客户端档案兼容不同交易软件（钱龙、国泰海通、中泰）。
...
- 现有客户端：`qianlong`（钱龙模拟期权宝，默认）、`guotai_haitong`（国泰海通证券期权宝）、`zhongtai`（中泰证券期权宝）。
