...
===========================
功能:
    遍历所有交易所(上证/深证)的所有策略类型
    - 无数据的策略自动跳过
    - 有数据的策略执行第一个组合申报

使用方法:
    1. 打开钱龙旗舰版,登录交易账号
    2. 修改下方参数(默认使用全部交易所和策略)
...
ORDER_QTY = int(os.environ.get("GUI_ORDER_QTY", "1"))  # 委托数量(组合申报弹窗中的数量)，可由 GUI 参数配置覆盖
...
def select_strategy(win, strategy: str):
    """选择策略类型。"""
    combo = win.child_window(auto_id="9040", control_type="ComboBox")
    combo.wait("ready", timeout=5)

    try:
        combo.select(strategy)
...
def click_first_list_row_and_combine(win, wait_timeout: float = 5) -> bool:
    """
    点击 ListBox 第一条数据并执行组合申报。
    ...
    total_groups = total_rows // 2
    first_row = real_rows[0]
    print(f"[OK] ListBox 共 {total_rows} 条数据,共 {total_groups} 组,点击第一条选中组合")
    first_row.click_input()
    time.sleep(0.3)

    # 点击"组合"按钮
    combo_btn = win.child_window(title="组合", auto_id="9058", control_type="Button")
    combo_btn.wait("ready", timeout=5)
    combo_btn.click_input()
    print("[OK] 已点击'组合'按钮")
    ...
    for _ in range(max_attempts):
        ...
        for elem in findwindows.find_elements(top_level_only=True):
            ...
            try:
                ...
                dlg = dlg_app.window(handle=hwnd)
                title = dlg.window_text() or ""
                if "组合申报" in title or "警告" in title:
                    dlg.set_focus()
                    time.sleep(0.1)
                    dlg.type_keys("{ENTER}", with_spaces=False)
                    print(f"[OK] '{title}'弹窗已回车确认")
                    found = True
                    time.sleep(0.3)
                    break
            except Exception:
...
def process_strategy(win, exchange: str, strategy: str) -> bool:
    """
    处理单个策略:
    1. 选择策略类型
    2. 检查是否有数据
    3. 有则执行组合申报
    返回 True 表示执行了组合, False 表示跳过
    """
    print(f"\n--- 处理: {exchange} | {strategy} ---")

    # 选择策略
    if not select_strategy(win, strategy):
        print(f"[--] 策略选择失败,跳过")
        return False
    ...
    # 有数据,执行组合
    print(f"[OK] 策略有数据,执行组合申报")
    if click_first_list_row_and_combine(win, wait_timeout=5):
    ...
    try:
        ...
        for exchange in EXCHANGES:
            ...
            for strategy in STRATEGIES:
                try:
                    if process_strategy(win, exchange, strategy):
                        total_executed += 1
                    else:
                        total_skipped += 1
                except Exception as e:
