# -*- coding: utf-8 -*-
"""
交易系统设置 - 自动拆单设置界面自动化测试
============================================
功能：
    打开"交易系统设置"对话框，进入"自动拆单设置"标签页，
    逐项读取每个参数的当前值（复选框状态、数值等），
...
AUTO_ID = {
    # 一、随机拆单
    "随机拆单_启用": "2046",
    "随机拆单_限价_如果下单张数大于": "2199",
    "随机拆单_限价_以最少": "2189",
    "随机拆单_限价_最多": "2185",
    "随机拆单_限价_间隔毫秒": "2194",
    "随机拆单_市价_如果下单张数大于": "2200",
    "随机拆单_市价_以最少": "2190",
    "随机拆单_市价_最多": "2186",
    "随机拆单_市价_间隔毫秒": "2195",
    ...
    # 二、固定拆单
    "固定拆单_启用": "2032",
    "固定拆单_限价_如果下单张数大于": "2201",
    "固定拆单_限价_以每次": "2187",
    "固定拆单_限价_间隔毫秒": "2196",
    "固定拆单_市价_如果下单张数大于": "2202",
    "固定拆单_市价_以每次": "2188",
    "固定拆单_市价_间隔毫秒": "2197",
}
...
# ============ Win32 消息加速（与 1_委托设置.py / 2_期权设置.py 一致）============
def _win32_user32():
...
def test_random_split(dlg, result: SettingsTestResult):
    """测试一、随机拆单"""
    print("\n--- [1/2] 随机拆单 ---")

    enabled = get_checkbox_state_by_id(dlg, AUTO_ID["随机拆单_启用"])
    result.add_result("随机拆单_启用", enabled, STANDARD_VALUES["随机拆单_启用"])

    # 无论是否启用，随机拆单下的参数组件均可见，直接读取比对
    keys = [
        "随机拆单_限价_如果下单张数大于",
        "随机拆单_限价_以最少",
        "随机拆单_限价_最多",
    ...
    for key in keys:
        actual = get_edit_value_by_id(dlg, AUTO_ID[key])
        result.add_result(key, actual if actual is not None else 0,
                          STANDARD_VALUES[key])


def test_fixed_split(dlg, result: SettingsTestResult):
    """测试二、固定拆单"""
    print("\n--- [2/2] 固定拆单 ---")

    enabled = get_checkbox_state_by_id(dlg, AUTO_ID["固定拆单_启用"])
    result.add_result("固定拆单_启用", enabled, STANDARD_VALUES["固定拆单_启用"])

    # 无论是否启用，固定拆单下的参数组件均可见，直接读取比对
    keys = [
        "固定拆单_限价_如果下单张数大于",
        "固定拆单_限价_以每次",
        "固定拆单_限价_间隔毫秒",
    ...
    try:
        ...
        for ctrl in descendants:
            try:
                ...
                if ctrl_type in ("CheckBox", "Edit", "Spinner", "Button", "Text"):
                    extra = ""
...
def main():
    ...
    try:
        ...
        # 6. 执行各项测试
        test_random_split(dlg, result)
        test_fixed_split(dlg, result)

        # 7. 截图
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        screenshot_path = os.path.join(OUTPUT_DIR, RESULT_SUBDIR, f"自动拆单设置_{timestamp}.png")
        ...
        # 9. 保存报告
        report_path = os.path.join(OUTPUT_DIR, RESULT_SUBDIR, f"自动拆单设置测试报告_{timestamp}.txt")
