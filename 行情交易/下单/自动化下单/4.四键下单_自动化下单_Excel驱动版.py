...
使用方法:
    1. 打开钱龙旗舰版,登录交易账号
    2. 准备 Excel 文件,填入下单配置(菜单列填写"四键下单")
...
# 东吴证券期权宝：四键下单嵌在「其他下单方式」容器下，需用完整路径导航；
# 其它客户端仍是根级单层路径（由 Excel"菜单"列提供，默认"四键下单"）。
# Excel"菜单"列仍按"四键下单"填写（read_excel 据此过滤行），此处仅覆盖
# 面板切换用的树路径。
_CLIENT_ID = os.environ.get("GUI_CLIENT_ID", "")
PANEL_PATH = r"\其他下单方式\四键下单" if _CLIENT_ID == "dongwu" else ""
...
ACTION_BUTTONS = {
    ...
    "买平": "18070",   # 实际 title: 买平
    "卖平": "18072",   # 实际 title: 卖平
}
...
def read_excel(path: str) -> list:
    """读取 Excel 文件,返回配置列表(只读取菜单列为"四键下单"的行)。"""
    ...
    wb.close()
    print(f"[OK] 读取 Excel: {path}, 共 {len(rows)} 条配置(菜单列过滤: 四键下单)")
    return rows
...
def click_action_button(main_hwnd, action: str):
    """点击下单动作按钮(买开/卖开/买平/卖平),用真实鼠标点击(确保触发软件下单逻辑)。"""
...
def main():
    try:
        ...
        tree_item = first_config.get("菜单", "四键下单")
        if not tree_item:
            tree_item = "四键下单"
        # 东吴：四键下单嵌在「其他下单方式」容器下，覆盖为完整路径
        if PANEL_PATH:
